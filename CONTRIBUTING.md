...
This repo packages [Tor](https://gitlab.torproject.org/tpo/core/tor/) for StartOS.
...
Tor is installed via `apk add tor` from the Alpine base image — there is no pinned upstream tag. The Tor version that ships is whatever Alpine carries.

1. Bump the Alpine base image in `Dockerfile` (and any related layer pins) to pick up a newer Tor.
